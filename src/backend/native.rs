...
//! slot in alongside the `ddcutil` one — this is the first of them, and
//! `backend::macos` is its sibling for Apple Silicon/Intel Macs; the two
//! share `backend::mccs_shared`'s capability-database logic (name/type
//! resolution, the `WELL_KNOWN` fallback table) rather than duplicating
//! it, since that's exactly where this backend's two early bugs lived.
//!
