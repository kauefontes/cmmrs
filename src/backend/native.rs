...
use mccs_db::{Access, Database, ValueType as DbValueType};
...
fn detect(slot: &Mutex<Vec<Entry>>) -> Result<Vec<Display>> {
    ...
    for dir_entry in dir_entries {
        ...
        found.push(Entry {
            ...
            mfg_id,
            model,
            vcp_version: mccs::Version::new(version.sh, version.sl).to_string(),
            raw: None,
            db: None,
        });
    }
...
/// `(name, recognized)` for a feature code, per the MCCS spec database —
/// `recognized` means the spec (not just this monitor's capability string)
/// knows what the code means, same distinction `ddcutil` draws.
fn describe(db: &Database, code: u8) -> (Option<String>, bool) {
    match db.get(code).and_then(|d| d.name.clone()) {
...
fn get_vcp(entry: &mut Entry, code: u8) -> Result<FeatureReading> {
    ensure_caps(entry)?;
    let descriptor = entry.db.as_ref().unwrap().get(code).cloned();
